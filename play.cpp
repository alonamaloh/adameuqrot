...
int main(int argc, char** argv) {
  std::string tb_dir = "/home/alvaro/claude/damas-gp";
  std::string nn_model = "";
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--model" && i + 1 < argc) {
      ...
      std::cout << "Usage: " << argv[0] << " [options]\n"
                ...
                << "  --tb PATH         Tablebase directory (default: /home/alvaro/claude/damas-gp)\n"
  ...
  while (true) {
    ...
    if (moves.empty()) {
      // Gana-pierde: side with no moves WINS
      std::cout << (white_to_move ? "White" : "Black") << " has no moves - ";
      std::cout << (white_to_move ? "White" : "Black") << " wins! (gana-pierde)\n";
      break;
    }
