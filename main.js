...
const MATCH_STATS_KEY = 'gana-pierde-match-stats';
...
async function init() {
    ...
    try {
        ...
        try {
            const nnData = await loadNNModelFile('./models/v0.bin');
        ...
        // Load opening book
        try {
            const bookResponse = await fetch('./opening-440285b5afa5.cbook');
            if (bookResponse.ok) {
...
const PREFS_KEY = 'gana-pierde-preferences';
...
async function showDownloadDialog(type = 'dtm') {
    ...
    try {
        // Close worker's sync handles, then clear existing tablebases
        // (they may be from a different variant)
        if (statusEl) statusEl.textContent = 'Limpiando tablas anteriores...';
        await engine.closeTablebases();
