...
self.onmessage = function(e) {
    ...
    try {
        switch (type) {
            ...
            case 'closeTablebases':
                // Close all sync access handles so OPFS files can be deleted
                for (const [, handle] of tbSyncHandles) {
                    try { handle.close(); } catch (e) {}
                }
                tbSyncHandles.clear();
                tbFileSizes.clear();
                chunkCache.clear();
                tablebasesAvailable = false;
                for (const [, handle] of cwdlSyncHandles) {
                    try { handle.close(); } catch (e) {}
                }
                cwdlSyncHandles.clear();
                cwdlFileSizes.clear();
                cwdlAvailable = false;
                response.success = true;
                break;
